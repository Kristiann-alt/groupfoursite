...
class Users(models.Model):
    ...
    gender = models.ForeignKey(Genders, on_delete=models.CASCADE) # gender_id BIGINT NOT NULL // FOREIGN KEY(gender_ide) REFERENCES tbl_genders(gender_id) ON DELETE CASCADE
    birth_date = models.DateField(blank=False) # birth_date DATE NOT NULL
    address = models.CharField(max_length=255, blank=False) # contact_number VARCHAR(255) NOT NULL
    contact_number = models.CharField(max_length=55, blank=False) # contact_number VARCHAR(55) NOT NULL
    email = models.EmailField(max_length=55, blank=False) # email VARCHAR(55) NOT NULL
